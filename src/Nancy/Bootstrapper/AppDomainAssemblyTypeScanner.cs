namespace Nancy.Bootstrapper
{
	...
	public static class AppDomainAssemblyTypeScanner
	{
		...
		/// Nancy core assembly
		/// </summary>
		private static readonly Assembly nancyAssembly = typeof (NancyEngine).Assembly;
		...
		private static List<Func<Assembly, bool>> assembliesToScan;

		/// <summary>
		/// The default assemblies for scanning.
		/// Includes the nancy assembly and anything referencing a nancy assembly
		/// </summary>
		public static List<Func<Assembly, bool>> DefaultAssembliesToScan = new List<Func<Assembly, bool>>
																		   {
																			   x => x == nancyAssembly,
																			   x =>
																			   {
		...
		/// </summary>
		public static List<Func<Assembly, bool>> AssembliesToScan
		{
			private get { return assembliesToScan ?? (assembliesToScan = DefaultAssembliesToScan); }
			set
			{
				assembliesToScan = value.ToList();
				UpdateTypes();
			}
		}
		...
		public static void AddAssembliesToScan(params string[] assemblyNames)
		{
			...
			AssembliesToScan = AssembliesToScan.Union(scanningPredicates).ToList();
		}
		...
		{
			AssembliesToScan = AssembliesToScan.Union(predicates).ToList();
		}
		...
		public static void LoadAssemblies(string containingDirectory, string wildcardFilename)
		{
			...
			var unloadedAssemblies = Directory
				.GetFiles(containingDirectory, wildcardFilename)
				.Where(f => !existingAssemblyPaths.Contains(f, StringComparer.OrdinalIgnoreCase));
		...
		private static void UpdateAssemblies()
		{
			Assemblies = AppDomain.CurrentDomain
								  .GetAssemblies()
								  .Where(assembly => AssembliesToScan.Any(asm => asm(assembly)))
								  .Where(assembly => !assembly.IsDynamic)
								  .Where(assembly => !assembly.ReflectionOnly)
								  .ToList();

			//var loaded = Assemblies.SelectMany(a => a.LoadAndReturnReferencedAssemblies()).Distinct().ToList();

			//var newAssemblies = loaded.Except(Assemblies);
		...
		{
			var returnTypes = Types.Where(type.IsAssignableFrom);
