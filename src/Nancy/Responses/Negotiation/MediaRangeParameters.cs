namespace Nancy.Responses.Negotiation
{
	...
	public class MediaRangeParameters : IEnumerable<KeyValuePair<string, string>>
	{
		...
		{
			var p1 = this.parameters.Where(p => p.Key != "q").OrderBy(p => p.Key);
			var p2 = other.parameters.Where(p => p.Key != "q").OrderBy(p => p.Key);
			var equal = p1.SequenceEqual(p2);

			return equal && (this.Quality == other.Quality);
		}
		...
		/// <summary>
		/// Returns the quality, which is the parameter named 'q' (returning 1 if there parameter is not set)
		/// </summary>
		public decimal Quality
		{
			...
			{
				var q = this["q"];
				return q == null ? 1 : decimal.Parse(q);
			}
		}
